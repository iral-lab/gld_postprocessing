...
def main():
    ...
    for bag_file in os.listdir(ARGS.data_dir):
        # create necessary path to store image data
        instance, ext = os.path.splitext(bag_file)

        print('Extracting data from %s instance...' % instance)

        image_path = os.path.join(ARGS.data_dir, 'data', instance, 'image')
        os.makedirs(image_path)
           
        bag = rosbag.Bag(os.path.join(ARGS.data_dir, bag_file), 'r')
        ...
        for topic, msg, t in bag.read_messages(topics=['/rgb/image_raw']):
            ...
            cv2.imwrite(image_path, '%s_%d.png' % (instance, frame / ARGS.frame_rate)), cropped_image)
